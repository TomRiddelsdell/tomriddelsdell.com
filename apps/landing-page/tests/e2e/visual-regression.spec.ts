...
test.describe('Visual Regression', () => {
  ...
  test('homepage should match visual snapshot', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const timeElements = document.querySelectorAll('[data-testid="current-year"]')
...
test.describe('CSS Application Verification', () => {
  test('hero background should have correct styles', async ({ page }) => {
    ...
    const position = await heroSection.evaluate((el) => 
  ...
  test('images should have correct dimensions', async ({ page }) => {
    ...
    const profileImg = page.locator('img[alt*="Tom Riddelsdell"], img[alt*="riddelsdell"]').first()
