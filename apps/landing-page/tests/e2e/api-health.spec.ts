...
test.describe('API Health Endpoints', () => {
  ...
  test('health endpoint should include observability info', async ({
    request,
  }) => {
    const response = await request.get('/api/health')
    const data = await response.json()
  ...
  test('metrics endpoint should include performance metrics', async ({
    request,
  }) => {
    const response = await request.get('/api/metrics')
    const data = await response.json()
  ...
  test('404 on non-existent API routes should return proper JSON error', async ({
    request,
  }) => {
...
test.describe('API Endpoint Availability', () => {
  ...
  for (const endpoint of criticalEndpoints) {
    test(`${endpoint.name} endpoint should be accessible`, async ({
      ...
      expect(
        response.status(),
        `${endpoint.name} (${endpoint.path}) should return 200`
      ).toBe(200)
    })
  }
})
